...
const LeaderboardCard = (props) => {
  return (
    <div className="lg:w-350 w-327 h-auto rounded-md border text-primary-blue focus:ring-2 hover:border-main">
      <h1 className="text-2xl leading-9 font-semibold text-primary-blue text-center pt-8">
        Leaderboard
      </h1>
      {props.topUsers &&
        props.topUsers.users.map((topUser) => {
          return (
            <div
      ...
      <div className="mt-2 pb-10">
        {props.realtiveUsers &&
          props.realtiveUsers.users.map((realtiveUser) => {
            let active = false;
            if (
